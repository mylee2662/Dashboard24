...
void Dash::UpdateDisplay(Adafruit_RA8875 tft)
{
    ...
    // cycle based on time
    int fl_wheel_speed = (millis() / 100) % 200;
    int fr_wheel_speed = (millis() / 2000) % 200;
    int num_states = 3;
    int curr_drive_state = (millis() / 1000) % num_states;
    int imd_stautus = (millis() / 1000) % 2;
#endif
    ...
    if (this->prev_fl_wheel_speed != fl_wheel_speed)
        DrawWheelSpeed(tft, fl_wheel_speed, wheel_speed_startX, wheel_speed_startY);

    this->prev_fl_wheel_speed = fl_wheel_speed;

    DrawBar(tft, 0, &prev_temp_bar_y, 100, 25, 0, 50, &prev_temp_bar_level);

    // draw IMD status
    DrawIMDStatus(tft, 8, 8, imd_stautus, 32);
...
void Dash::DrawWheelSpeed(Adafruit_RA8875 tft, float wheel_speed, int startX, int startY)
{
    ...
    // fill in the space
    tft.fillRect(startX - 128, startY, 256, 72, RA8875_WHITE);
    ...
    while (rounded_wheel_speed > 0)
    {
        ...
        // fill in the area to the left of the digit
        tft.fillRect(startX - digit_spacing, startY, digit_spacing, 72, RA8875_WHITE);

        startX -= (digit_spacing + char_width);
    }
}
...
void Dash::DrawDriveState(Adafruit_RA8875 tft, int startX, int startY, int curr_drive_state, int squareSize)
{
    ...
    {
        // White out space
        tft.fillRect(startX, startY, 64, 80, RA8875_WHITE);
        ...
        case 1:
            ...
            for (int x = 1, y = 0; x < 6 && y < 9; x++, y += 2)
            {
                ...
                if (y + 1 < 9)
                { // Check if there's room to move down one more row
...
void Dash::DrawIMDStatus(Adafruit_RA8875 tft, int startX, int startY, int imd_status, int squareSize)
{
    ...
    default:
        if (imd_status >= 0)
        {
            // this is the resistance value
            status = "IMD: " + std::to_string(imd_status) + " KOhms";
        }
        else
        {
            status = "IMD: Unknown";
    ...
    {
        tft.drawChar(startX + i * squareSize, startY, status[i], RA8875_BLACK, RA8875_WHITE, squareSize / 6);
    }
}